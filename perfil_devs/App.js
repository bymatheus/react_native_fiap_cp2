...
const App = () => {
  ...
  const [currentURL, setCurrentURL] = useState(
    "https://i.pinimg.com/736x/b1/5e/ed/b15eedbdafbbdbca3249e3942f4faf3b.jpg"
  );
  ...
  const pesquisarDev = async () => {
    try {
      if (!currentUser)
        return alert("Preencha corretamente o login do usuário");
      setHasResult(false);
      const response = await axios(
        `https://api.github.com/users/${currentUser}`
      );
      const { avatar_url, company, followers, following, name, login } =
        response.data;
