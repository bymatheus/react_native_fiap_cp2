...
class App extends Component {
  ...
  async calc() {
    ...
    if (this.state.fromValue == this.state.toValue)
      return this.setState({ result: this.state.value });
    const returnStr = (value) =>
      value == 0 ? "BRL" : value == 1 ? "EUR" : value == 2 ? "USD" : "BTC";
    const url = `https://economia.awesomeapi.com.br/json/last/${returnStr(
      this.state.fromValue
    )}-${returnStr(this.state.toValue)}`;
    try {
      const response = await axios(url);
      const { ask } = Object.values(response.data)[0];
      if (!ask)
        return this.setState({ result: "Erro na conversão dos valores" });
      if (this.state.value == parseFloat(1))
        return this.setState({ result: parseFloat(ask) });
      this.setState({
        result:
          parseFloat(ask) > 1
            ? this.state.value / parseFloat(ask)
            : this.state.value * parseFloat(ask),
      });
    } catch (error) {
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.title}> Conversor de Moedas 2.0 </Text>
        ...
        <View style={styles.containerInput}>
          ...
          <Picker
            ...
            onValueChange={(itemValue, itemIndex) =>
              this.setState({ fromValue: itemValue })
            }
          >
            {optionsItems}
          </Picker>
        ...
        <View style={styles.containerInput}>
          ...
          <Picker
            ...
            onValueChange={(itemValue, itemIndex) =>
              this.setState({ toValue: itemValue })
            }
          >
            {optionsItems}
          </Picker>
